...
/// Build the query-result envelope:
/// `{"ok": true, "columns": [...], "rows": [[...]]}`.
///
/// This matches the daemon's NDJSON query-response shape exactly so agents
/// need no extra parsing logic when switching between direct CLI and daemon use.
